<template>
    ...
    <template v-if="info !== null">
      <div class="col-12 bg-info text-center text-white sticky-top">
        ...
        <span class="fa-2">退票单详情</span> <small>单号:{{info.id}}</small>
      </div> 
      <div class="card col-12">
        <dl class="row mt-2">
          ...
          <dt class="col-4 text-right px-0">退票原因</dt>
          <dd class="col-8">
            {{info.reason}}
          </dd>
...
import { searchRefundOrderDetail } from '../api/order.js'

export default {
  computed: {
    info () { return this.$store.state.refundOrderInfo }
  },
  mounted: function () {
    var id = this.$route.params.id
    if (id !== undefined && this.info === null) {
      this.refreshRefundOrderDetail(parseInt(id))
    }
  },
  methods: {
    back: function () {
    ...
    refreshRefundOrderDetail: function (id) {
      var self = this

      self.showLoading('退票单查询中...')
      console.log(id)
      searchRefundOrderDetail(id,
        (jsonResult) => {
          if (jsonResult !== null && jsonResult.id === id) {
            self.$store.commit('setRefundOrderInfo', jsonResult)
          }
        },
